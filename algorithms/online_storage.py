...
class OnlineStorage(object):
    def __init__(self,
        ...
        self.action_log_probs_exp = None

        # values and returns
        self.value_preds = torch.zeros(num_steps + 1, num_processes, 1)
        self.value_preds_exp = torch.zeros(num_steps + 1, num_processes, 1)
        self.returns = torch.zeros(num_steps + 1, num_processes, 1)
        self.returns_exp = torch.zeros(num_steps + 1, num_processes, 1)
    ...
    def to_device(self):
        ...
        self.value_preds_exp = self.value_preds_exp.to(device)
        self.returns = self.returns.to(device)
        self.returns_exp = self.returns_exp.to(device)
    ...
    def insert(self,
        ...
        if isinstance(value_preds_exp, list):
            self.value_preds_exp[self.step].copy_(value_preds_exp[0].detach())
        else:
            self.value_preds_exp[self.step].copy_(value_preds_exp.detach())
    ...
    def compute_returns_exp(self, next_value, use_gae, gamma, tau, use_proper_time_limits=True):

        rewards = self.intrinsic_rewards.clone()

        self._compute_returns(next_value=next_value, rewards=rewards, value_preds=self.value_preds_exp,
                              returns=self.returns_exp,
                              gamma=gamma, tau=tau, use_gae=use_gae, use_proper_time_limits=use_proper_time_limits)
    ...
    def before_update_exp(self, policy):
        latent = utl.get_latent_for_policy(self.args,
                                           latent_sample=torch.stack(
                                               self.latent_samples[:-1]) if self.latent_samples is not None else None,
                                           latent_mean=torch.stack(
        ...
        _, action_log_probs, _ = policy.evaluate_actions(self.prev_state[:-1],
                                                         ...
                                                         self.tasks[:-1] if self.tasks is not None else None,
                                                         self.actions)
        self.action_log_probs_exp = action_log_probs.detach()
    ...
    def feed_forward_generator_exp(self,
                               advantages,
                               num_mini_batch=None,
                               mini_batch_size=None):
        num_steps, num_processes = self.rewards_raw.size()[0:2]
        ...
        for indices in sampler:
            ...
            value_preds_batch = self.value_preds_exp[:-1].reshape(-1, 1)[indices]
            return_batch = self.returns_exp[:-1].reshape(-1, 1)[indices]

            old_action_log_probs_batch = self.action_log_probs_exp.reshape(-1, 1)[indices]
            if advantages is None:
                adv_targ = None
            else:
                adv_targ = advantages.reshape(-1, 1)[indices]
