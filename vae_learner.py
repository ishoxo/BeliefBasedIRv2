...
class VAELearner:
    ...
    def __init__(self, args):
        ...
        self.exp_label = 'pretrain_VAE'
        self.args = args
        #make sure we are trainnig on either, task ,reward, or dynamics
        assert(int(args.decode_task) + int(args.decode_reward) + int(args.decode_state) == 1)
        if args.decode_task:
            self.exp_label += '_task'
        if args.decode_reward:
            self.exp_label += '_reward'
        if args.decode_state:
            self.exp_label += '_state'
        # ipdb.set_trace()
        ...
        self.logger = TBLogger(self.args, self.exp_label)
    ...
    def train(self):
        ...
        for self.iter_idx in range(self.num_updates):
            if self.iter_idx % 25 == 0:
                print('Starting update: {}'.format(self.iter_idx))
            ...
            for step in range(self.args.policy_num_steps):
                ...
                self.policy_storage.insert(
                    ...
                    latent_mean=latent_mean,
                    latent_logvar=latent_logvar,
                    intrinsic_reward=rew_raw,
                    value_preds_exp=value
                )
