...
class MetaLearner:
    ...
    def __init__(self, args):
        ...
        self.intrinsic_reward_type =  self.args.IR_type
        ...
        self.policy_exp = self.initialise_policy()
    ...
    def train(self):
        ...
        for self.iter_idx in range(self.num_updates):
            ...
            for step in range(self.args.policy_num_steps):
                ...
                with torch.no_grad():
                    value, value_exp, action = utl.select_action_both(
                        args=self.args,
                        explore=True,
                        policy=self.policy,
                        policy_exp=self.policy_exp,
                        state=prev_state,
                        belief=belief,
                        task=task,
                        deterministic=False,
                ...
                # self.policy_storage.latent_mean[-1]
                # self.policy_storage.latent_logvar[-1]
                intrinsic_reward_raw = self.compute_IR(mu=latent_mean, logE=latent_logvar,
                                                       m=self.policy_storage.latent_mean[-1], logS =self.policy_storage.latent_logvar[-1],
                                                       prev_sample=self.policy_storage.latent_samples[-1],
                                                       state=prev_state, next_state=next_state,
                                                       rewards=rew_raw, actions=action, tasks=task)


                intrinsic_reward_raw = intrinsic_reward_raw.unsqueeze(dim=-1)
    ...
    def compute_IR(self, mu, logE, m, logS, prev_sample, state, next_state, rewards, actions, tasks):
        """
        :param mu: current mean
        :param logE: current logvar
        :param m: prev means
        :param logS: prev logvar
        :return: IR
        """
        ...
        # logE = all_logvars[1:]
        # logS = all_logvars[:-1]
        gauss_dim = self.args.latent_dim

        if self.intrinsic_reward_type == 'KL':

            IR = 0.5 * (torch.sum(logS, dim=-1) - torch.sum(logE, dim=-1) - gauss_dim +
                       torch.sum(1 / torch.exp(logS) * torch.exp(logE), dim=-1) + ((m - mu) /
                        torch.exp(logS) * (m - mu)).sum(dim=-1))

        elif self.intrinsic_reward_type == "EUC":
            IR = torch.norm(mu - m, dim=1)

        elif self.intrinsic_reward_type == "VAR":
            prev_var = torch.exp(logS)
            var = torch.exp(logE)
            IR = torch.sum(prev_var - var, dim=1)

        elif self.intrinsic_reward_type == "REW_TD":
            if self.args.disable_stochasticity_in_latent:
                latent_state = torch.cat((m, logS), dim=1)
            else:
                latent_state = prev_sample

            with torch.no_grad():
               pred_rewards = self.vae.reward_decoder.forward(latent_state, next_state)

            IR = torch.norm(pred_rewards - rewards, dim=1)

        elif self.intrinsic_reward_type == "TRANS_TD":
            if self.args.disable_stochasticity_in_latent:
                latent_state = torch.cat((m, logS), dim=1)
            else:
                latent_state = prev_sample
            ...
            with torch.no_grad():
               state_pred = self.vae.state_decoder.forward(latent_state, state, actions)

            if self.args.state_pred_type == 'deterministic':
                IR = (state_pred - next_state).pow(2).mean(dim=-1)
                ...
                IR = -m.log_prob(next_state).mean(dim=-1)


        elif self.intrinsic_reward_type == "TASK_TD":

            with torch.no_grad():
                task_pred = self.vae.task_decoder(prev_sample)

            IR = (task_pred - tasks).pow(2).mean(dim=-1)
    ...
    def get_value_exp(self, state, belief, task, latent_sample, latent_mean, latent_logvar):
        latent = utl.get_latent_for_policy(self.args, latent_sample=latent_sample, latent_mean=latent_mean, latent_logvar=latent_logvar)
        return self.policy_exp.actor_critic.get_value(state=state, belief=belief, task=task, latent=latent).detach()

    def update_explorer(self, state, belief, task, latent_sample, latent_mean, latent_logvar):

        if self.iter_idx >= self.args.pretrain_len and self.iter_idx > 0:
            # bootstrap next value prediction
            with torch.no_grad():
                next_value = self.get_value_exp(state=state,
                                            belief=belief,
                                            task=task,
                                            latent_sample=latent_sample,
                                            latent_mean=latent_mean,
            ...
            self.policy_storage.compute_returns_exp(next_value, self.args.policy_use_gae, self.args.policy_gamma,
                                                self.args.policy_tau,
                                                use_proper_time_limits=self.args.use_proper_time_limits)

            # update agent (this will also call the VAE update!) \mke sure this does not call VAE update
            policy_train_stats = self.policy_exp.update(
    ...
    def update(self, state, belief, task, latent_sample, latent_mean, latent_logvar):
        ...
        if self.iter_idx >= self.args.pretrain_len and self.iter_idx > 0:
            ...
            policy_exp_train_stats = self.policy_exp.update_exp(
                policy_storage=self.policy_storage,
                encoder=None,
                rlloss_through_encoder=False,
                compute_vae_loss=False)
    ...
    def log(self, run_stats, train_stats, start_time):
        ...
        if (self.iter_idx + 1) % self.args.save_interval == 0:
            ...
            for idx_label in idx_labels:
                ...
                torch.save(self.policy_exp.actor_critic, os.path.join(save_path, f"policy{idx_label}.pt"))
