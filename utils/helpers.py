...
def select_action_both(args,
    ...
    if explore:
        action = policy_exp.act(state=state, latent=latent, belief=belief, task=task, deterministic=deterministic)
        if isinstance(action, list) or isinstance(action, tuple):
            value_pred_exp, action = action
        else:
            value_pred_exp = None

        value_pred, _ = policy.actor_critic.forward(state, latent, belief, task)
        ...
        action = policy.act(state=state, latent=latent, belief=belief, task=task, deterministic=deterministic)
        if isinstance(action, list) or isinstance(action, tuple):
            value_pred, action = action
        else:
            value_pred = None

        value_pred_exp, _ = policy_exp.actor_critic.forward(state, latent, belief, task)



    action = action.to(device)
    return value_pred, value_pred_exp, action
