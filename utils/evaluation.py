...
def compute_IR(args, mu, logE, m, logS):
    gauss_dim = args.latent_dim
    ...
    if args.IR_type == 'KL':
        ...
        Exception('IR type not supported')
...
def evaluate(args,
    ...
    intrinsic_returns_per_episode = torch.zeros((num_processes, num_episodes + 1)).to(device)
    ...
    for episode_idx in range(num_episodes):
        ...
        for step_idx in range(num_steps):
            ...
            if encoder is not None:
                ...
                new_latent_sample, new_latent_mean, new_latent_logvar, new_hidden_state = utl.update_encoding(encoder=encoder,
                                                                                              next_obs=state,
                                                                                              action=action,
                                                                                              reward=rew_raw,
                                                                                              done=None,
                                                                                              hidden_state=hidden_state)
                intrinsic_reward_raw = compute_IR(args=args, mu=latent_mean,
                                                    logE=latent_logvar,
                                                    m= latent_sample,
                                                    logS=latent_mean)
                latent_sample, latent_mean, latent_logvar, hidden_state = new_latent_sample, new_latent_mean, \
                                                                          new_latent_logvar, new_hidden_state
...
def visualise_behaviour(args,
    ...
    for i in range(len(latent_means)):
        episode_latent_means = latent_means[i]
        episode_latent_logvars = latent_logvars[i]

        mu = episode_latent_means[1:]
        m = episode_latent_means[:-1]
        logE = episode_latent_logvars[1:]
        logS = episode_latent_logvars[:-1]
        IR = compute_IR(mu=mu, logE=logE, m=m, logS=logS, args=args)
        episode_IRs.append(IR)

    # ipdb.set_trace()


    # plot_rewards(episode_rewards)
    plot_rewards(episode_rewards, episode_IRs, image_folder, iter_idx)
...
def plot_rewards(rewards, IR, image_folder, iter_idx):
    rewards = [item.cpu().numpy().copy() for item in rewards]
    # ipdb.set_trace()
    plt.figure(figsize=(12, 12))
    plt.suptitle("Episode Rewards", fontsize=35)
    for i in range(len(rewards)):
        plt.subplot(2, 2, i+1)
        plt.title('Episode {}'.format(i))
        plt.plot(rewards[i])
        plt.grid(True)
    plt.savefig('{}/{}_Episode_Rewards'.format(image_folder, iter_idx))

    int_rewards = [item.cpu().numpy().copy() for item in IR]
    # ipdb.set_trace()
    plt.figure(figsize=(12, 12))
    plt.suptitle("Episode Intrinsic Rewards", fontsize=35)
    for i in range(len(int_rewards)):
        plt.subplot(2, 2, i + 1)
        plt.title('Episode {}'.format(i))
        plt.plot(int_rewards[i])
        plt.grid(True)
    plt.savefig('{}/{}_Episode_IR'.format(image_folder, iter_idx))
